 ...
 */
function sendMail($destinataire, $sujet, $corpsHtml) {
    global $config;

    if (!isSmtpConfigured()) {
        return logMailInstead($destinataire, $sujet, $corpsHtml, 'journalise');
    }

    try {
        smtpSend($destinataire, $sujet, $corpsHtml);
        logMailInstead($destinataire, $sujet, $corpsHtml, 'envoye');
        return true;
    } catch (Exception $e) {
        logError('Envoi email échoué : ' . $e->getMessage());
        logMailInstead($destinataire, $sujet, $corpsHtml, 'echec', $e->getMessage());
        return false;
    }
}

function logMailInstead($destinataire, $sujet, $corpsHtml, $statut, $erreur = null) {
    try {
        $pdo = initDatabase();
 ...
 */
function smtpSend($destinataire, $sujet, $corpsHtml) {
    global $config;
